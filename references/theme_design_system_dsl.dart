...
/// Stand-in for an uploaded asset's storage path.
///
/// Asset storage paths embed the project id, so you never write one by hand.
/// Real code uploads the file and passes what the upload returns:
///
/// ```dart
/// final sdk = FlutterFlowAI(apiKey: apiKey);
/// final regular = await sdk.assets.uploadFile(
///   projectId,
///   File('Inter-Regular.ttf'),
/// );
/// CustomFontFile(fontFilePath: regular.storagePath);
/// ```
const _kAssetPrefix = 'projects/example-project-id/assets/uploaded';
...
void buildThemeDesignSystemApp(App app) {
  ...
  // ---------------------------------------------------------------------------
  // 5. Custom font asset — uploaded TTFs
  // ---------------------------------------------------------------------------
  ...
  // is false.
  //
  // `fontFilePath` is the font asset's STORAGE PATH, not a Flutter bundle path.
  // Upload the files first and pass what the upload returns:
  //
  //   final sdk = FlutterFlowAI(apiKey: apiKey);
  //   final regular = await sdk.assets.uploadFile(
  //     projectId,
  ...
  //   ... CustomFontFile(fontFilePath: regular.storagePath)
  //
  // The literal paths below stand in for those upload results.
  app.customFont(
    'Inter',
    files: [
      const CustomFontFile(fontFilePath: '$_kAssetPrefix/Inter-Regular.ttf'),
      CustomFontFile(
        fontFilePath: '$_kAssetPrefix/Inter-Medium.ttf',
        weight: FFFontWeight.w500,
      ),
      CustomFontFile(
        fontFilePath: '$_kAssetPrefix/Inter-Bold.ttf',
        weight: FFFontWeight.w700,
      ),
      const CustomFontFile(
        fontFilePath: '$_kAssetPrefix/Inter-Italic.ttf',
        italic: true,
      ),
    ],
  );
  ...
  // ---------------------------------------------------------------------------
  // 6. Custom icon family — uploaded icon font
  // ---------------------------------------------------------------------------
  ...
  app.customIconFamily(
    'BrandIcons',
    iconFilePath: '$_kAssetPrefix/brand_icons.ttf',
    icons: const [
