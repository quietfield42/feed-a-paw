...
///
/// Two pub.dev packages are used here on purpose:
///   * `http` — consumed by a custom action (network call).
///   * `intl` — consumed by a custom widget (currency formatting).
///
/// A third, `animations`, is pulled straight from the `flutter/packages` Git
/// monorepo to show the Git-source shape (`app.pubGitDependency` /
/// `addPubGitDependency`) next to its pub.dev siblings.
library;
...
void buildPubPackageShowcase(App app) {
  ...
  // A package consumed straight from a Git repository — the shape for a fork
  // or an unreleased fix. `path` selects one package inside the monorepo and
  // `ref` pins the exact commit so the build is reproducible (a branch name
  // works too, but moves under you). Pub clones the whole repository, so keep
  // the target small in real projects. Nothing below imports this package; it
  // is here to sit next to the pub.dev declarations.
  app.pubGitDependency(
    'animations',
    url: 'https://github.com/flutter/packages.git',
    ref: 'bb97da8ec627466fdb45cdd36e2c0e35dee40e9e', // animations-v2.0.11
...
void buildPubPackageEdit(App app) {
  ...
  app.raw((project) {
    ...
    // Same guard for a Git source. To move a dependency that is already on
    // pub.dev onto a fork or an unreleased commit, swap the `add*` for
    // `updatePubGitDependency(project, name: ..., url: ..., ref: ...)`.
    if (findPubDependency(project, name: 'animations') == null) {
      addPubGitDependency(
        project,
        name: 'animations',
        url: 'https://github.com/flutter/packages.git',
...
''',
  ...
  // widget without redeclaring it, place `CustomWidget(...)`
  // directly and use generated typed SDK handles for the surrounding page and
  // widget targets:
  //
  //   CustomWidget('ReceiptCard', arguments: {'total': 42.0});
  //
  // The compiler resolves parameters against `findCustomWidget(project, ...)`
  // at compile time.
  // To evolve an existing definition and bind its placed instance without a
  // raw proto mutation, use the generated handle plus a typed property patch:
  //
  //   app.editCustomWidget(ff.CustomWidgets.receiptCard, (widget) {
  //     widget.ensureParam('currency', string, defaultValue: 'USD');
  //     widget.replaceCode(updatedReceiptCardCode);
  //   });
  //   app.editPage(ff.Pages.receiptPage, (page) => page.update(
  //     ff.Pages.receiptPage.widgets.byKey('Container_receipt').single,
  //     (patch) => patch.arguments({'currency': State('currency')})));
